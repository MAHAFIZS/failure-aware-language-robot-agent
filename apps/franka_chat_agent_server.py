...
def validate_plan(plan: dict[str, Any]) -> list[str]:
    ...
    for index, step in enumerate(steps, start=1):
        ...
        # Occupancy-aware rearrangement may move one cube
        # temporarily to a buffer and later to its final target.
...
while simulation_app.is_running():
    ...
    if active_step is None:
        ...
        placing_held_object = (
            active_step["tool"] == "place_on_object"
            and held_object_name == object_name
        )

        if placing_held_object:
            controller._event = 5
            controller._t = 0.0
            controller.resume()

            print(
                f"[Chat Agent] Resuming with held "
                f"{held_object_name} from event "
                f"{controller.get_current_event()}"
            )

        if (
            held_object_name is not None
            and active_step["tool"] == "place_on_object"
            and held_object_name != object_name
        ):
            step_results.append(
                {
                    ...
                    "success": False,
                    "failure_type": (
                        "different_object_already_held"
                    ),
                    "held_object": held_object_name,
                }
            )
    ...
    if controller.is_done():
        if placing_held_object:
            print(
                f"[Chat Agent] Released held object "
                f"{held_object_name} at destination."
            )
