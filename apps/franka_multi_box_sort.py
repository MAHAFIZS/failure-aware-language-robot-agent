...
parser = argparse.ArgumentParser(
    description="Sort colored cubes into matching colored bins."
)

parser.add_argument(
    "--instruction",
    default="Sort all colored boxes into their matching bins.",
)
...
custom_tray_path = (
    ...
    / "objects"
    / "custom_sorting_tray"
    / "custom_sorting_tray_physics.usd"
)

if not custom_tray_path.exists():
    raise FileNotFoundError(
        f"Custom tray not found: {custom_tray_path}"
    )

add_reference_to_stage(
    usd_path=str(custom_tray_path),
    prim_path="/World/CustomSortingTray",
)
...
tray_prim = stage.GetPrimAtPath(
    "/World/CustomSortingTray"
)

tray_xform = UsdGeom.XformCommonAPI(tray_prim)

tray_xform.SetTranslate(
    Gf.Vec3d(-0.55, 0.0, 0.025)
)

tray_xform.SetRotate(
    Gf.Vec3f(0.0, 0.0, 90.0),
    UsdGeom.XformCommonAPI.RotationOrderXYZ,
)

print(
    "[Isaac Sort] Custom tray placed at "
    "(-0.25, 0.0, 0.035)"
)
...
sorting_items = [
    {
        ...
        "target": np.array([-0.25, 0.28, CUBE_Z]),
    },
    {
        "label": "green",
...
for item in sorting_items:
    ...
    # A thin colored tray marks the matching bin.
    world.scene.add(
        VisualCuboid(
            name=f"{label}_bin",
            prim_path=f"/World/{label.capitalize()}Bin",
            position=np.array(
                [
                    item["target"][0],
                    item["target"][1],
...
print("\n[Isaac Sort] Instruction:")
print(args.instruction)

print("\n[Isaac Sort] Planned sequence:")
...
while simulation_app.is_running():
    ...
    if step_count >= args.max_steps:
        print(
            "[Isaac Sort] Maximum step count reached."
        )
        break
    ...
    if controller.is_done():
        ...
        print(
            f"[Isaac Sort] {item['label']} cube finished: "
        ...
        if current_item_index >= len(
            ...
            print(
                "[Isaac Sort] Sorting sequence completed."
            )
        else:
            controller.reset()
...
result = {
    "schema_version":
        "contacttrace_isaac_sort_episode_v0.1",
    "simulator":
        "isaac_sim",
    "robot":
        "franka_panda",
    ...
    "seed":
        int(args.seed),
    "task_type":
        "multi_object_color_sort",
    "planner":
        "rule_based_sort_sequence",
    ...
    "success":
        all_sorted,
    "failure_type":
        None if all_sorted else "sorting_incomplete",
    "duration_sec":
        duration_sec,
    "simulation_steps":
        step_count,
    ...
    "simulation_engineering": {
        ...
        "validation": {
            ...
            "constraint_violations":
                0,
            "all_objects_completed":
                len(completed_items)
                == len(sorting_items),
        },
        "asset_pipeline": {
            "robot_asset":
                "NVIDIA Franka Panda USD",
            "environment_assets":
                "Procedural Isaac Sim primitives",
            "version_control_ready":
                True,
        },
    },
}
...
print("\n[Isaac Sort] Final ContactTrace result:")
print(
    json.dumps(
        result,
        indent=2,
...
print(
    f"\n[Isaac Sort] Result saved: {result_path}"
)
