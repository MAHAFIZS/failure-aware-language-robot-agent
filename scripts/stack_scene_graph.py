...
def plan_place_on_object(
    ...
    support_blockers = find_all_objects_above(
        graph,
        support_object,
    )

    # The requested moving object should not be moved to a
    # temporary buffer if it is already above the support.
    support_blockers = [
        name
        for name in support_blockers
        if name != moving_object
    ]

    blockers: list[str] = []

    # Remove top objects first and avoid duplicates.
    for name in support_blockers + moving_blockers:
    ...
    for blocker, target in zip(
        ...
        reason = (
            f"Clear {support_object} before placing "
            f"{moving_object} on it."
            if blocker in support_blockers
            else f"Clear {moving_object} before moving it."
        )
    ...
    steps.append(
        {
            ...
            "reason": (
                f"{support_object} is clear, so place "
                f"{moving_object} on it."
            ),
        }
    )
    ...
    return {
        ...
        "support_object": support_object,
        "moving_object_blockers": moving_blockers,
        "support_object_blockers": support_blockers,
