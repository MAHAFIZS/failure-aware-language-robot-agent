...
from procedural_letter_planner import (
    build_procedural_digit_plan,
    build_procedural_letter_plan,
    extract_requested_digit,
    extract_requested_letter,
)
from typing import Any

from occupancy_aware_rearrangement import (
    expand_occupancy_aware_plan,
)

from stack_scene_graph import (
    KNOWN_OBJECTS,
    plan_pick_with_unstacking,
...
5. release_object
Use for commands such as "release it", "open the gripper",
"drop the held cube", or "let go".
...
def main() -> None:
    ...
    # Fast procedural route for clear single-digit commands.
    requested_digit = extract_requested_digit(
        args.instruction
    )
    ...
    looks_like_digit_request = (
        requested_digit is not None
        and any(
            keyword in args.instruction.lower()
            for keyword in digit_keywords
        )
    )

    if looks_like_digit_request:
        print(
            f"[Planner] Fast procedural-digit route: "
            f"{requested_digit}",
            flush=True,
        )

        plan = build_procedural_digit_plan(
            args.instruction,
            requested_digit,
        )
        ...
        if plan.get("approved"):
            if not status_file.exists():
                ...
                plan["validation_errors"] = [
                    "The current Isaac scene is unavailable."
                ]
            else:
                ...
                try:
                    plan = expand_occupancy_aware_plan(
                        positions,
                        plan,
                    )
        ...
        plan["parsed_intent"] = {
            "intent": "procedural_digit",
            "digit": requested_digit,
            "reason": "Direct single-digit extraction.",
        }
    ...
    # Fast deterministic route for clear single-letter commands.
    # This avoids waiting for Ollama reasoning when the requested
    # capital letter can be extracted directly.
    requested_letter = extract_requested_letter(
        args.instruction
    )
    ...
    if looks_like_letter_request:
        print(
            f"[Planner] Fast procedural-letter route: "
            f"{requested_letter}",
            flush=True,
        )
        ...
        plan["parsed_intent"] = {
            "intent": "procedural_letter",
            "letter": requested_letter,
            "reason": "Direct single-letter extraction.",
        }
    ...
    if intent == "pick_object":
        if not status_file.exists():
            ...
            requested_object = parsed.get("moving_object")

            if not requested_object:
                plan = rejected_plan(
                    instruction,
                    intent,
                    "The requested cube color was not identified.",
                    parsed,
                )
            else:
                plan = plan_pick_with_unstacking(
                    positions,
                    requested_object,
                )
